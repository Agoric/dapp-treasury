...
const Adjust = ({
  ...
  const unliened =
    ...
    AmountMath.subtract(
      accountState.bonded,
      liened ?? AmountMath.makeEmpty(brand),
    );

  // Automatically adjust collateral if "max" is selected.
  useEffect(() => {
    ...
    if (collateralAction === 'lock' && lockedDelta?.value !== unliened?.value) {
      ...
      const lienedToUse = liened ?? AmountMath.makeEmpty(brand);
      const maxLocked = floorMultiplyBy(newBorrowed, invertRatio(borrowLimit));
      if (lienedToUse.value < maxLocked.value) return;

      const newLockedDelta = AmountMath.subtract(lienedToUse, maxLocked);
  ...
  // Automatically adjust debt if "max" is selected.
  useEffect(() => {
    ...
    if (debtAction === 'borrow') {
      ...
      const debtToUse = debt ?? AmountMath.makeEmpty(debtBrand);
      if (debtToUse.value > newDebtLimit.value) return;

      const newDebtDelta = AmountMath.subtract(newDebtLimit, debtToUse);
  ...
  // Reset form when switching between "borrow" and "repay" tabs.
  useEffect(() => {
  ...
  const isLoanInProgress = LoanStatus.PROPOSED === loan?.status;
      ...
      <div className={classes.collateralForm}>
        <div className={classes.collateralInfo}>
          {collateralAction === 'lock'
            ? `Unliened: ${displayAmount(
                unliened ?? AmountMath.makeEmpty(brand),
              )} BLD`
            : `Liened: ${displayAmount(
                liened ?? AmountMath.makeEmpty(brand),
              )} BLD`}
        </div>
        <NatPurseAmountInput
    ...
    <>
      <Paper elevation={3} className={classes.root}>
        ...
        <div className={classes.confirm}>
          ...
          <ConfirmOfferTable
            locked={liened ?? AmountMath.makeEmpty(brand)}
